...
///         break
///     }
public func _await<T>(promise: Promise<T>) throws -> Resolution<T>  {
...
/// - Returns: the promise's resolution
/// - Throws: COError
public func _await<T>(closure: @escaping () -> Promise<T> ) throws -> Resolution<T> {
    return try _await(promise: closure())
}
...
/// - Throws: COError
public func _await<T>(channel: Chan<T>) throws -> T {
    if let _ = Coroutine.current() {
...
/// - Returns: the promise's resolution
/// - Throws: COError
public func _await<T>(closure: @escaping () -> Chan<T> ) throws -> T {
    return try _await(channel: closure())
}
